...
class DeploymentController extends Controller
{
    public function show(): Response
    {
        ...
        $heading = $isUpdate ? 'Update Timeline Curator' : 'Install Timeline Curator';
        $description = $isUpdate
            ? 'Run the database migrations included with the uploaded update package.'
            : 'This one-time installer creates or updates the configured database.';
        $ownerFields = $isUpdate ? '' : <<<'HTML'
...
<h1>{$heading}</h1>
<p>{$description} Enter the deployment token generated with the release package.</p>
...
HTML);
    ...
    public function install(Request $request): Response
    {
        ...
        $expectedHash = $this->expectedTokenHash();
        ...
        try {
            ...
            if ($isUpdate) {
                File::delete($this->updateTokenHashPath());
            }

            $heading = $isUpdate ? 'Database update complete' : 'Database installation complete';
            $nextStep = $isUpdate
                ? 'The one-use update token has been disabled. Return to the Timeline and retry curation.'
                : 'Set <code>WEB_INSTALLER_ENABLED=false</code>, then create your Timeline account and authorize the Codex plugin. Auth0 is not required.';
...
HTML);
    ...
    private function expectedTokenHash(): string
    {
        if ($this->isUpdateMode()) {
            return trim((string) File::get($this->updateTokenHashPath()));
        }

        return (string) config('deployment.web_installer_token_hash');
    }

    private function isUpdateMode(): bool
    {
        return File::exists($this->updateTokenHashPath());
    }

    private function updateTokenHashPath(): string
    {
        return storage_path('app/deployment-update-token-hash');
    }
